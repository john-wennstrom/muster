 ...
 * the tool sets each mode gets. Nothing else under src/ launches the Pi executable.
 */
...
export async function runAgent(options: RunAgentOptions): Promise<AgentRun> {
  ...
  try {
    ...
    return await runPiProcess({
      ...
      prompt: [
        options.prompt,
        `Task mode: ${task.mode}. Declared read scopes: ${JSON.stringify(task.reads)}. Declared write scopes: ${JSON.stringify(task.writes)}.`,
        task.mode === "read" ? "Do not modify repository files." : "Keep repository changes within the declared write scopes.",
      ].join("\n\n"),
