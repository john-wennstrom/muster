/**
 * pi-process.ts — the `pi --mode json -p` child process: how the running pi binary is found again,
 * how a child's extensions are resolved, and how its JSON events stream into an AgentRun.
 * Everything about launching a child that is not a role, a scope or a broker lives here.
 */
...
import type { ResolvedChildRuntime, ChildAccess } from "./child-runtime.ts";
import { briefArg, runOk, type AgentRun } from "./run-record.ts";
 ...
 */
export function runPiProcess(opts: {
