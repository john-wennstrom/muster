...
const fixtureNames = ["direct-change", "bounded-change", "architectural-change"] as const;
...
type LoadedFixture = Awaited<ReturnType<typeof loadFixture>>;
...
async function createHarness(fixtureName: typeof fixtureNames[number]) {
...
function approveCurrentArtifacts(harness: Awaited<ReturnType<typeof createHarness>>): void {
...
async function reachVerified(harness: Awaited<ReturnType<typeof createHarness>>): Promise<void> {
...
describe("fixture-driven change lifecycle", () => {
  for (const fixtureName of fixtureNames) {
    test(`${fixtureName} progresses from status through VERIFIED`, async () => {
      ...
      const policy = orchestrationPolicy(decision.classification, {
      ...
      expect(Object.values(policy.mandatory).every(Boolean)).toBeTrue();
      expect(policy.optional.debate).toBe(fixtureName === "architectural-change");
