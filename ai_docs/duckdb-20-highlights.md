...
_TL;DR: DuckDB v2.0 is coming this fall. In this post, we preview its headline features: DuckDB as a server, triggers, the VARIANT type, asynchronous I/O, a new SQL parser, a new storage format, and much more._

DuckDB v2.0 will be named “Cyanoptera” after the [cinnamon teal](https://en.wikipedia.org/wiki/Cinnamon_teal) _(Anas cyanoptera),_ a strikingly reddish-brown duck found in the western Americas.
...
DuckDB is moving rather quickly, and we can only cover a small fraction of the changes here. Condensing all new features down to a shortlist is always a fight over what gets in, and yes, we know that what follows is technically a listicle _(Ten Things Coming to DuckDB v2.0, Number Eight Will Shock You)._ We are not proud of the format, but it works, so here it is, starting with the SQL-level features and working down into the engine.

## [1\. DuckDB as a Server: Quack and `CONNECT`](https://duckdb.org/2026/08/17/duckdb-20-highlights\#1-duckdb-as-a-server-quack-and-connect)

DuckDB has been an in-process database since day one. But people have asked us – very persistently – for a client/server mode, and we have finally caved. The [`quack`extension](https://github.com/duckdb/duckdb-quack) implements DuckDB's native protocol for talking to other DuckDBs. It was [released as a preview](https://duckdb.org/2026/05/12/quack-remote-protocol.html) shortly before DuckCon #7, graduates to stable in v2.0, and it is a big part of where DuckDB is headed: any DuckDB process can serve its databases over the network, and any other DuckDB can attach to it and route queries there using the new `CONNECT` statement. For example:

#### [DuckDB server](https://duckdb.org/2026/08/17/duckdb-20-highlights\#duckdb-server)
...
#### [DuckDB client](https://duckdb.org/2026/08/17/duckdb-20-highlights\#duckdb-client)
...
## [2\. `VARIANT` Becomes a First-Class Citizen](https://duckdb.org/2026/08/17/duckdb-20-highlights\#2-variant-becomes-a-first-class-citizen)

The `VARIANT` type shipped in [DuckDB v1.5](https://duckdb.org/2026/03/09/announcing-duckdb-150.html), and the way to think about it is JSON on steroids. Basically, imagine if JSON were fast. Like JSON, a `VARIANT` column can store differently-shaped data in every row. Unlike JSON, it is not a text format: DuckDB automatically detects the common structure hidden in your semi-structured data and “shreds” it, so it compresses well in storage and executes fast in queries, all without you ever declaring a schema. This makes `VARIANT` a natural fit for real-time log ingestion, where streams of JSON-ish records share structure but evolve over time.

In v2.0, this pipeline works end to end: shredded execution straight from storage ( [#20912](https://github.com/duckdb/duckdb/pull/20912)), extraction pushdown into scans ( [#22478](https://github.com/duckdb/duckdb/pull/22478)), shredded `VARIANT` reading _and_ writing for Parquet, and a family of `variant_*` functions:
...
## [3\. Triggers](https://duckdb.org/2026/08/17/duckdb-20-highlights\#3-triggers)
...
## [4\. SQL Dialect Additions](https://duckdb.org/2026/08/17/duckdb-20-highlights\#4-sql-dialect-additions)
...
The new **variable syntax** ( [#21194](https://github.com/duckdb/duckdb/pull/21194)) lets you write `$x` anywhere an expression is allowed, no more [`getvariable(...)` verbiage](https://duckdb.org/docs/current/sql/functions/utility.html#getvariablevariable_name):
...
And **recursive CTEs with [`USING KEY`](https://duckdb.org/2025/05/23/using-key.html) aggregation** ( [#19481](https://github.com/duckdb/duckdb/pull/19481)) enable iterative algorithms in pure SQL, backed by the rewritten recursive CTE engine described below:
...
## [5\. Asynchronous I/O](https://duckdb.org/2026/08/17/duckdb-20-highlights\#5-asynchronous-io)

Interacting with object stores like S3 is central to the DuckDB experience: your data has to come from somewhere, and it often sits in object storage. DuckDB has long been able to read from object stores in parallel, but synchronous access placed a limit on how fast this could go. DuckDB v2.0 introduces asynchronous I/O throughout the engine. We described the design in detail in [a dedicated blog post](https://duckdb.org/2026/07/31/asynchronous-io.html).
...
## [6\. Faster Queries Across the Board](https://duckdb.org/2026/08/17/duckdb-20-highlights\#6-faster-queries-across-the-board)
...
How much faster can this get? Here is a microbenchmark you can run on a laptop: single-source reachability over a graph with one million edges, written as a plain [recursive CTE](https://duckdb.org/docs/current/sql/query_syntax/with.html).
...
Row-group pruning has been massively expanded: [min-max indexes (zone maps)](https://duckdb.org/docs/current/sql/indexes.html#min-max-index-zonemap%20%}) and [Parquet Bloom filters](https://duckdb.org/2025/03/07/parquet-bloom-filters-in-duckdb.html) now skip data for structs, lists, decimals, UUIDs, `IN` filters, and even function predicates:
...
Query planning also becomes **partition-aware** ( [#22336](https://github.com/duckdb/duckdb/pull/22336)). [Lakehouse formats](https://duckdb.org/docs/current/lakehouse_formats.html) (DuckLake, Iceberg and plain Hive-partitioned Parquet on S3) are all partitioned, and exploiting that partitioning is often the difference between scanning a dataset and skipping most of it. In v2.0, the planner and optimizer take full advantage of existing partitioning, and partitioned writes have been reworked as well ( [#22225](https://github.com/duckdb/duckdb/pull/22225), [#22620](https://github.com/duckdb/duckdb/pull/22620)).

## [7\. Storage Format v2.0](https://duckdb.org/2026/08/17/duckdb-20-highlights\#7-storage-format-v20)

DuckDB v2.0 bumps the default [storage format version](https://duckdb.org/docs/current/internals/storage.html) to v2.0.0 ( [#22875](https://github.com/duckdb/duckdb/pull/22875)).
...
## [9\. Timezones, Calendars, and Collations Without ICU](https://duckdb.org/2026/08/17/duckdb-20-highlights\#9-timezones-calendars-and-collations-without-icu)
...
## [10\. Write Extensions Once, Host Them Yourself](https://duckdb.org/2026/08/17/duckdb-20-highlights\#10-write-extensions-once-host-them-yourself)

Extensions are one of the best things about DuckDB, but today, most of them, including our own, build against the [unstable C++ API](https://duckdb.org/docs/current/clients/cpp.html). That means extension authors have to re-target and rebuild for every DuckDB release, and community extensions can silently disappear when their authors stop keeping up. DuckDB v2.0 broadens the _stable_ C API far enough that extensions can be written once, built once, published once, and keep working, essentially until the end of time.
...
## [Bonus: DuckDB Foundation – Advisory Board](https://duckdb.org/2026/08/17/duckdb-20-highlights\#bonus-duckdb-foundation--advisory-board)
...
## [Final Thoughts](https://duckdb.org/2026/08/17/duckdb-20-highlights\#final-thoughts)
...
There have been more than 10,000 commits by many contributors since we released v1.5. We would like to thank our community for the detailed issue reports, feedback, and contributions that shaped this release. If you want a taste before the fall, the [preview builds](https://duckdb.org/install/preview.html) have most of these features today, and if something breaks, you know where [the issue tracker](https://github.com/duckdb/duckdb/issues) is.
...
[Reconciling JSON in DuckDB, One Patch at a Time](https://duckdb.org/2026/08/18/reconciling-json.html "Reconciling JSON in DuckDB, One Patch at a Time")

![Reconciling JSON in DuckDB, One Patch at a Time](https://duckdb.org/images/blog/thumbs/json-patch.svg)
...
![](https://duckdb.org/images/blog/authors/mustafa_khan_atlan.jpg)

Mustafa Khan

[Thank You for 40&nbsp;000 Stars on GitHub](https://duckdb.org/2026/08/05/github-40k-stars.html "Thank You for 40&nbsp;000 Stars on GitHub")

![Thank You for 40&nbsp;000 Stars on GitHub](https://duckdb.org/images/blog/thumbs/github-40k-stars.svg)
...
[Asynchronous I/O in DuckDB: Work, Thread, Work](https://duckdb.org/2026/07/31/asynchronous-io.html "Asynchronous I/O in DuckDB: Work, Thread, Work")

![Asynchronous I/O in DuckDB: Work, Thread, Work](https://duckdb.org/images/blog/thumbs/async.svg)
...
![](https://duckdb.org/images/blog/authors/pedro_holanda.jpg)
