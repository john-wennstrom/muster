...
- [x] 5.1 Add `prompts` to the `files` array in `package.json`. Write `prompts/README.md` (one page) documenting the two file kinds, the front-matter variable declaration, the strictness rules, optional blocks, the YAML question format including `repeat`, and how to regenerate goldens with `tests/prompts/capture.ts` when wording changes on purpose. Link it from `AGENTS.md` conventions. Run `bun run docs:check`.
