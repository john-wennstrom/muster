...
- [x] 3.2 Move the review decisions (`review.task_focus`, `review.extraction`, `review.triage`) the same way into `src/judgment/decisions/review-task-focus.ts`, `review-extraction.ts` and `review-triage.ts`, with their state builders and presenters. Update imports in `src/review/**` and `src/change/**`.
