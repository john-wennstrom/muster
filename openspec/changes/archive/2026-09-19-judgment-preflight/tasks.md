...
- [x] 4.1 Add the planning preflight row to the per-call-site table in `docs/security.md`, naming the request text and the retrieved source excerpts with their paths, and noting the excerpt and candidate caps; verify the documentation checks pass.
...
- [x] 4.2 Run the full validation set and compare pass and fail counts against the recorded pre-existing platform baseline, confirming that preflight is identical to today's with judgment disabled and that the agent's parsing hardening is unchanged.
